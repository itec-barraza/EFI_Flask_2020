...
def show_signup_form():
    ...
    if form.validate_on_submit():
        ...
        if user is not None:
            error = f'This e-mail {email} is already in use'
        else:
            ...
            send_email(subject='Bienvenid@ a Sarasa Games',
                       sender=current_app.config['DONT_REPLY_FROM_EMAIL'],
                       recipients=[email, ],
                       text_body=f'Hola {name}, bienvenid@ a Sarasa Games',
                       html_body=f'<p>Hola <strong>{name}</strong>, bienvenid@ a Sarasa Games</p>')
